...
class EditSavedPlanPage extends StatefulWidget {
  const EditSavedPlanPage({super.key, this.template});

  final WorkoutPlanTemplate? template;

  @override
  State<EditSavedPlanPage> createState() => _EditSavedPlanPageState();
}

class _EditSavedPlanPageState extends State<EditSavedPlanPage> {
  ...
  final WorkoutPlanTemplateRepository _planRepository = WorkoutPlanTemplateRepository();
  bool _loading = false;
  bool _saving = false;
  ...
  void initState() {
    ...
    if (widget.template != null) {
      _planNameController.text = widget.template!.name;
      _planned.addAll(widget.template!.exercises);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: FittaAppBar(
        title: widget.template == null ? 'Yeni Plan Oluştur' : 'Planı Düzenle',
      ),
                  ...
                  child: Column(
                    ...
                    children: [
                      if (!isSearching && _planned.isEmpty)
                        ...
                        Text('Plan İçeriği (${_planned.length})',
  ...
  Future<void> _savePlan() async {
    ...
    if (name.isEmpty) {
      Get.snackbar('Uyarı', 'Lütfen plan adı girin', snackPosition: SnackPosition.BOTTOM);
      return;
    }
    if (_planned.isEmpty) {
    ...
    try {
      ...
      await _planRepository.savePlanTemplate(userId: _userId, template: template);
      Get.back();
